...
class GeneralLinearUpdater {
   private:
    ...
    void applyInternalMuUpdate(const double mu_numerator_new,
   ...
   public:
    GeneralLinearUpdater(const double mu_numerator, const double mu_denominator,
                         const double sigma_state_value,
                         const double last_observation, const double n_obs);
