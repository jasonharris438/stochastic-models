...
void check_function_status(const int& status, const std::vector<int> ignore_codes) {
    if (status && std::find(ignore_codes.begin(), ignore_codes.end(), status) ==
                      ignore_codes.end()) {
        if (status == GSL_EMAXITER) {
            std::cerr << "error: " << gsl_strerror(status) << std::endl;
            throw IntegrationMaxIterationError("Maximum number of subdivisions reached during numerical integration.");
        }
            ...
            std::cerr << "error: " << gsl_strerror(status) << std::endl;
            throw NoSolutionError("Root finding solver failed due to no solution at a single point.");
        }
            ...
            std::cerr << "error: " << gsl_strerror(status) << std::endl;
            throw SingularityError("A singularity was encountered during numerical integration.");
        }
