...
const double
GeneralLinearUpdater::initialSigmaState(const double& sigma, const int& n_obs) {
...
const double GeneralLinearUpdater::updateSigma(
    const double& value, const double& expected_x
) {
  if (n_obs < 0) {
