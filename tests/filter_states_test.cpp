...
const matrix<double> create_boost_transition_matrix() {
    ...
    for (int i{0}; i < std_vectors.size(); i++) {
        std::move(std_vectors.at(i).begin(), std_vectors.at(i).end(),
...
TEST(KalmanFilterTest, PredictedObservationCalculateMeanTest) {
    ...
    const PredictedObservation predicted_observation(observation_matrix,
                                                     observation_offset);
...
TEST(KalmanFilterStateTest, KcaStatessetInitialStateTest) {
    ...
    std::copy(current_state_mean.begin(), current_state_mean.end(),
              current_state_mean_vector.begin());
    const std::vector<double> expected_current_state_mean{10.288741828687053,
                                                          0.0, 0.0};
...
TEST(KalmanFilterStateTest, KcaStatesupdatePredictedStateTest) {
    ...
    std::copy(predicted_state_mean.begin(), predicted_state_mean.end(),
              predicted_state_mean_vector.begin());
    const std::vector<double> expected_predicted_state_mean{10.301048359829961,
                                                            0.0, 0.0};
    ...
    const std::vector<std::vector<double>> predicted_state_covariance =
        copy_matrix_elements_to_vector(
            kca_states.getPredictedStateCovariance());
...
TEST(KalmanFilterStateTest, KcaStatesupdateCurrentStateTest) {
    ...
    std::copy(kca_states.getPredictedObservationMean().begin(),
    ...
    EXPECT_EQ(predicted_observation_mean_vector,
              expected_predicted_observation_mean)
    ...
    EXPECT_EQ(predicted_observation_covariance_vector,
    ...
    std::copy(new_current_state_mean.begin(), new_current_state_mean.end(),
              new_current_state_mean_vector.begin());
    const std::vector<double> expected_current_state_mean{10.3000765492722, 0,
                                                          0};
