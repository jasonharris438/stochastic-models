...
class GeneralLinearLikelihood : public LikelihoodCalculatuor {
   public:
     ...
     * series.
     */
    const double calculateLeadLagInnerProduct(
     ...
     */
    const double calculateSigmaKernel(const u_int32_t& n_observations,
     ...
     */
    const double calculateSigma(const std::vector<double>& data,
    ...
    const double calculateConditionalVariance(const double& sigma,
                                              const double& mu) const;
};
