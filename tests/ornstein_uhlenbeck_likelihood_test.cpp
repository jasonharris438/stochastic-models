...
// Tests the Ornstein-Uhlenbeck likelihood class returns valid updated model parameter values.
...
TEST(OrnsteinUhlenbeckLikelihoodCalculateTest, ComponentsTest) {
  ...
  EXPECT_LE(abs(roundToDecimals(components.lead_sum, 2) - 4.0), tolerance)
      << "OrnsteinUhlenbeckLikelihood not calculating correct value for the lead sum component.";
  // Expect equality for the lag sum component.
  EXPECT_LE(abs(roundToDecimals(components.lag_sum, 2) - 3.5), tolerance)
      << "OrnsteinUhlenbeckLikelihood not calculating correct value for the lag sum component.";
  // Expect equality for the lead sum squared component.
  EXPECT_LE(abs(roundToDecimals(components.lead_sum_squared, 4) - 4.125), tolerance)
      << "OrnsteinUhlenbeckLikelihood not calculating correct value for the lead sum squared component.";
    // Expect equality for the lag sum squared component.
  EXPECT_LE(abs(roundToDecimals(components.lag_sum_squared, 4) - 3.375), tolerance)
      << "OrnsteinUhlenbeckLikelihood not calculating correct value for the lag sum squared component.";
    // Expect equality for the lead-lag sum product component.
  EXPECT_LE(abs(roundToDecimals(components.lead_lag_sum_product, 2) - 3.25), tolerance)
      << "OrnsteinUhlenbeckLikelihood not calculating correct value for the lead-lag sum product component.";
    // Expect equality for the number of observations component.
  EXPECT_LE(abs(roundToDecimals(components.n_obs, 2) - 6.0), tolerance)
      << "OrnsteinUhlenbeckLikelihood not calculating correct value for the number of observations component.";
}

// Tests the Ornstein-Uhlenbeck likelihood class returns valid model component values
// after an update.
TEST(OrnsteinUhlenbeckLikelihoodUpdateTest, ComponentsTest) {
  ...
  // Expect equality for the lead sum component.
  EXPECT_LE(abs(roundToDecimals(updated_components.lead_sum, 2) - 4.75), tolerance)
      << "OrnsteinUhlenbeckLikelihood not calculating correct value for the lead sum component.";
  // Expect equality for the lag sum component.
  EXPECT_LE(abs(roundToDecimals(updated_components.lag_sum, 2) - 4.5), tolerance)
      << "OrnsteinUhlenbeckLikelihood not calculating correct value for the lag sum component.";
  // Expect equality for the lead sum squared component.
  EXPECT_LE(abs(roundToDecimals(updated_components.lead_sum_squared, 4) - 4.6875), tolerance)
      << "OrnsteinUhlenbeckLikelihood not calculating correct value for the lead sum squared component.";
    // Expect equality for the lag sum squared component.
  EXPECT_LE(abs(roundToDecimals(updated_components.lag_sum_squared, 4) - 4.375), tolerance)
      << "OrnsteinUhlenbeckLikelihood not calculating correct value for the lag sum squared component.";
    // Expect equality for the lead-lag sum product component.
  EXPECT_LE(abs(roundToDecimals(updated_components.lead_lag_sum_product, 2) - 4.0), tolerance)
      << "OrnsteinUhlenbeckLikelihood not calculating correct value for the lead-lag sum product component.";
    // Expect equality for the number of observations component.
